...
// Flat brand purple, not the masked "pattern" strip artwork, and not the
// lab's own accent color (which the layout sets as --strip-color) — a plain
// solid-colored band that reads as fixed "Saudi Experience Labs" chrome.
// --brand-purple-dark is never offered as a lab accent option (see
// BRAND_ACCENTS in brand-color-picker.tsx), so it can't collide with
// whatever color a given lab picked.
function ColumnBand() {
  return (
    <div
      className="h-1.5 w-72 shrink-0"
      style={{ backgroundColor: "var(--brand-purple-dark)" }}
      aria-hidden
    />
  );
}
